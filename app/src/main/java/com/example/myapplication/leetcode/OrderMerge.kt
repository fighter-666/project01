...
import com.example.myapplication.leetcode.linked.LinkedNode

/**
 * 两个有序链表的合并
 * 以其中一个有序链表作为结果集（firstLinked），两个链表头结点开始向后比较；
 * 1.结果集（firstLinked）的值小于等于非结果集（secondLinked）的值，结果集（firstLinked）继续向后走一步
 * 2.如果非结果集（secondLinked）链表的值小于结果集（firstLinked）的值，则结果集（secondLinked）的值插入到当前结果集（firstLinked）的前一位
 * 3.最后逐一比较之后，非结果集（secondLinked）还有剩余，则将剩余的节点注意添加到结果集（firstLinked）后面
 */
class OrderMerge {
    companion object{
        @JvmStatic
    ...
    private fun merge(firstLinked: LinkedNode<Int>?, secondLinked: LinkedNode<Int>?):LinkedNode<Int>?{
        ...
        var prev = result //结果集当前比较靠前的前一节点
